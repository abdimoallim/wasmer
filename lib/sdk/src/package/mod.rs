...
/// Events generated during the publish process.
#[derive(Debug, Clone)]
...
pub async fn publish_package<F>(
    client: &WasmerClient,
    manifest_path: &Path,
    opts: PublishOptions,
    mut progress: F,
) -> Result<PackageIdent, PublishError>
where
    F: FnMut(PublishProgress),
{
    progress(PublishProgress::Building);
    let manifest_str = std::fs::read_to_string(manifest_path).map_err(|e| {
        if e.kind() == std::io::ErrorKind::NotFound {
            PublishError::ManifestNotFound(manifest_path.to_path_buf())
        } else {
            PublishError::ManifestRead(e)
        }
    })?;
    ...
    let (ns, name) = {
        ...
        let namespace = opts
            ...
            .ok_or_else(|| PublishError::Api(anyhow::anyhow!("namespace missing")))?;
        let name = opts
            .name
            .clone()
    ...
    let version = opts
        ...
        .ok_or_else(|| PublishError::Api(anyhow::anyhow!("package version missing")))?;

    let package_name =
        name.ok_or_else(|| PublishError::Api(anyhow::anyhow!("package name missing")))?;
    ...
    wasmer_backend_api::query::tag_package_release(
        ...
        manifest
            ...
            .map(|r| {
                std::fs::read_to_string(manifest_path.parent().unwrap().join(r)).unwrap_or_default()
            })
            .as_deref(),
        manifest
            .package
